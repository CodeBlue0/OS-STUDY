...
.SCREENCLEARLOOP:               ; 화면을 지우는 루프
    ...
    jl .SCREENCLEARLOOP         ; SI 레지스터가 더 작다면 다음 위치로 이동
...
PRINTMESSAGE:
    ...
    mov ax, 0xB800          ; 비디오 메모리 시작 어드레스(0xB80000)를 세그먼트 레지스터 값으로 변환
    mov es, ax              ; ES 세그먼트 레지스터에 설정
    ...
    mov si, 160             ; 한 라인의 바이트 수(2*80 칼럼)를 SI 레지스터에 설정
