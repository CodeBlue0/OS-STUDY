...
BOOL kActivateKeyboard(void) 
{
    ...
    for (j = 0; j < 100; j++)
    {
        ...
        // 출력 버터(포트 0x00)에서 읽은 데이터가 ACK(0xFA)이면 성공
...
// 출력 버터(포트 0x60)에서 키를 읽음
BYTE kGetKeyboardScanCode(void)
{
    // 출력 버터(포트 0x60)에 데이터가 있을 때까지 대기
...
BOOL kChangeKeyboardLED(BOOL bCapsLockOn, BOOL bNumLockOn, BOOL bScrollLockOn)
{
    ...
    for (i = 0; i < 0xFFFF; i++)
    {
        // 입력 버터(포트 0x60)가 비었으면 커맨드 전송 가능
        if (kIsInputBufferFull() == FALSE)
        {
            break;
        }
    ...
    // LED 변경 값을 키보드로 전송하고 데이터가 처리가 완료될 때까지 대가
...
void KEnableA20Gate(void)
{
    ...
    for (i = 0; i < 0xFFFF; i++)
    {
        // 출력 버터(포트 0x60)가 차 있으면 데이터를 읽을 수 있음
        if (kIsOutputBufferFull() == TRUE)
        {
            break;
        }
