...
BOOL kProcessMouseData(void)
{
    ...
    else if (pstWindowManager->bWindowResizeMode == TRUE)
    {
        ...
        if ((pstWindowManager->stResizingWindowArea.iX2 < pstWindowManager->
                stResizingWindowArea.iX1) ||
        ...
        if ((pstWindowManager->stResizingWindowArea.iY2 < pstWindowManager->
                stResizingWindowArea.iY1) ||
...
BOOL kProcessEventQueueData(void)
{
    ...
    // 저장된 이벤트를 검사하면서 합칠 수 있는 이벤트는 하나도 만듦
    iEventCount = i;
