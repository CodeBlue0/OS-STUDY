...
kOutPortWord:
    ...
    mov rdx, rdi        ; RDX 레지스터에 파라미터 1(포트 번호)을 저장
    mov rax, rsi        ; RAX 레지스터를 파라미터 2(데이터)를 저장
...
kReadMSR:
    ...
    mov qword [rsi], rdx    ; 두 번째 파라미터의 pqwRDX에 RDX 레지스터의 값을 저장
    mov qword [rbx], rax    ; 세 번째 파라미터의 pqwRAX에 RAX 레지스터의 값을 저장
