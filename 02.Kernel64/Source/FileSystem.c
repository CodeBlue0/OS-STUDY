...
BOOL kInitializeFileSystem(void)
{
    ...
    {
        // 초기화가 성공하면 함수 포인터를 램디스크용 함수로 설정
...
DWORD kWriteFile(const void* pvBuffer, DWORD dwSize, DWORD dwCount, FILE* pstFile)
{
    ...
    while (dwWriteCount != dwTotalCount)
    {
        ...
        {
            // 전체 클러스트를 덮어쓰는 경우가 아니면 부분만 덮어서야 하므로 현재 클러스트를 읽음
...
int kRemoveFile(const char* pcFileName)
{
    ...
    // 다른 태스크에서 해당 파일을 열고 있는지 핸들 풀을 검색하여 환인 파일이 열려 있으면 삭제할 수 없음
...
BOOL kFlushFileSystemCache(void)
{
    ...
    kGetCacheBufferAndCount(CACHE_DATAAREA, &pstCacheBuffer,
        &iCacheCount);
    for (i = 0; i < iCacheCount; i++)
    {
        // 캐시의 내용이 변했다면 태그에 저장된 위치에 직접 씀
