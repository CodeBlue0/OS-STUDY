...
static void kInitializeTCBPool(void)
{
    ...
    kMemSet((TCBPOOLMANAGER*)TASK_TCBPOOLADDRESS, 0, sizeof(TCB) * TASK_MAXCOUNT);
...
static BYTE kFindSchedulerOfMinumumTaskCount(const TCB* pstTask)
{
    ...
    iCurrentTaskCount = 
        kGetListCount(&(gs_vstScheduler[pstTask->bAPICID].vstReadyList[bPriority]));
        ...
        if (kGetListCount(&(gs_vstScheduler[bCurrentAPICID].stWaitList)) > 0)
        {
            ...
            { 
                ...
                if (pstTask->qwFlags & TASK_FLAGS_PROCESS)
                {
                    ...
                    {
                        // 유저 레벨 프로세스라면 메뫼를 할당받았을 것이므로 할당받은
                        // 메모리를 삭제
