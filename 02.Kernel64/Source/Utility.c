    ...
    for (i = 0; i < iFormatLength; i++)
    {
        ...
        if (pcFormatString[i] == '%')
        {
            ...
            case 'f':
                ...
                // 셋째 자리에서 반올림 처리
                dValue += 0.005;
                // 소수점 둘째 자리에서부터 차례로 저장하여 버퍼를 뒤집음
...
// 밀리세컨드(mil;isecond) 동안 대기
void kSleep(QWORD qwMillisecond)
{
    QWORD qwLaskTickCount;

    qwLaskTickCount = g_qwTickCount;

    while((g_qwTickCount - qwLaskTickCount) <= qwMillisecond)
    {
        kSchedule();
    }
}
