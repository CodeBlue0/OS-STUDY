    ...
    int iX, int iY, COLOR stColor)
{
    ...
    // 픽셀 오프셋으로 계산하여 픽셀 출력
    *((COLOR*) pstMemoryAddress + (iWidth * iY) + iX) = stColor;
}
    ...
    int iX1, int iY1, int iX2, int iY2, COLOR stColor)
{
    ...
    // Y축 변화량이 X축 변화량보다 크다면 Y축을 중심으로 직선을 그림
    else 
    {
    ...
    const char* pcString, int iLength)
{
    ...
    for (k = 0; k < iLength; k++)
    {
        ...
        for (j = iStartYOffset; j < iOverlappedHeight; j++)
        {
            ...
            // 다음 라인으로 이동해야 하므로, 현재 Y 좌표에 화면의 너비만큼 더해줌
