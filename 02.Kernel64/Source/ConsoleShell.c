...
static void kWaitUsingPIT(const char* pcParameterBuffer)
{
    ...
    // 인터럽트 비활성화하고 PIT 컨트롤러를 통해 직접 시간을 측정
    kDisableInterrupt();
...
static void kFPUTestTask(void)
{
    ...
    iOffset = (pstRunningTask->stLink.qwID & 0xFFFFFFFF) * 2;
    iOffset = CONSOLE_WIDTH * CONSOLE_HEIGHT - 
        (iOffset % (CONSOLE_WIDTH * CONSOLE_WIDTH));
...
static void kStartSymmetricIOMode(const char* pcParameterBuffer)
{
    ...
    kPrintf("Change Sysmmetric I/O Mode Complete\n");
}
...
static void kTestSystemCall(const char* pcParameterBuffer)
{
    ...
    // 동적 할당 영역에 4Kbyte 메모리를 할당받아 레벨 태스크를 생성할 준비를 함
