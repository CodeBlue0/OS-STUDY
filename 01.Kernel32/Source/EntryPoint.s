...
PRINTMESSAGE:
    ...
    push esi                
    push edi                ; 함수에서 임시로 사용하는 레지스터로 함수의 마지막 부분에서
    push eax                ; 스택에 삽입된 값을 꺼내 원래 값으로 복원
    push ecx
    push edx
    ...
    mov esi, 160            ; 한 라인의 바이트 수(2*80 칼럼)를 SI 레지스터에 설정
    mul esi                 ; AX 레지스터와 SI 레지스터를 곱하여 화면 Y 어드레스 계산
    mov edi, eax            ; 계산된 화면 Y 어드레스를 DI 레지스터에 설정

    ; X 좌표를 이용해서 2를 곱한 후 최종 어드레스를 구함
    mov eax, dword [ebp+8]   ; 파라미터 1(화면 좌표 X)를 AX 레지스터에 설정
    mov esi, 2               ; 한 문자를 나타내는 바이트 수(2)를 SI 레지스터에 설정
    mul esi                  ; AX 레지스터와 SI 레지스터를 곱하여 화면 X 어드레스를 계산
...
.MESSAGELOOP:               ; 메시지를 출력하는 루프
    mov cl, byte [esi]       ; SI 레지스터가 가리키는 문자열 위치에서 한 문자를
                            ; CL 레지스터에 복사
    ...
    add esi, 1               ; SI 레지스터에 1을 더하여 다음 문자열로 이동
    add edi, 2               ; DI 레지스터에 2를 더하여 비디오 메모리의 다음 문자 위치로 이동
